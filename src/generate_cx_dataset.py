...
def _clamp_nps(value: float) -> int:
...
def generate_rows() -> list[dict[str, str]]:
    ...
    for i in range(1, N_ROWS + 1):
        ...
        # Likelihood-to-recommend (0-10): same latent driver as CSAT but its
        # own construct and noise, so NPS and CSAT correlate without being
        # mechanically identical.
        nps = _clamp_nps(11 * propensity - 0.5 + rng.gauss(0.0, 1.4))
...
`nps_score` (0-10 likelihood-to-recommend) is generated from the *same*
latent propensity as `satisfaction_score` but with its own offset and wider
noise (sd 1.4), so NPS and CSAT are correlated but not mechanically
identical — promoter/passive/detractor banding and NPS-by-segment are
therefore a genuine second lens, not a relabelling of CSAT.
